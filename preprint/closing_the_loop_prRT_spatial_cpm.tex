...
and the activity-feedback specification. Three executed studies are reported in the
Conclusion, each against a pre-declared protocol with negative-control gates: transport
caching, open-loop response, and closed-loop selection, all on a synthetic lesion geometry.
Every other quantity in this preprint is either a literature prior or a verified absence.
...
The framework is a specification. Three studies have executed parts of it on a synthetic
lesion geometry with the Potts Hamiltonian inactive, and the Conclusion reports them. The
framework as specified here, with active Potts dynamics on a measured activity map, has not
run, and no result of this manuscript depends on that full loop running. What has been executed and validated, in our prior work, is the one-way
...
calibration. It reports three executed studies on a synthetic geometry, each committed with
its protocol, code and verdict. The one-way transport foundation is
